...
|STT|Chỉ tiêu|Kiểu dữ liệu (theo QĐ4750)|Diễn giải (CV3360)|Xuất 4210|Xuất 4750[^2024-09-14]|
|:-------:|-------|-------|-------|-------|-------|
|1|stt|NUMERIC(10,0)|Số thứ tự bệnh nhân từ 1 đến hết|||
|2|ma_bn|VARCHAR(100)|Mã số BN quy định tại CSKCB|`=bang1.ma_bn`|`=bang1.ma_bn`|
|3|ho_ten|VARCHAR(255)|Họ tên người bệnh viết bằng chữ thường.|`=bang1.ho_ten`|`=bang1.ho_ten`|
|4|ngay_sinh|VARCHAR(12)|Ngày sinh ghi trên thẻ gồm 8 ký tự; 4 ký tự năm + 2 ký tự tháng + 2 ký tự ngày (nếu không có ngày sinh thì ghi năm sinh: 4 ký tự)|`ngay_sinh=LEFT(bang1.ngay_sinh,8)`[^2024-09-06-01]. Riêng đối với người bệnh chỉ có năm sinh `ngay_sinh=LEFT(bang1.ngay_sinh,4)`[^2024-09-13-01]|`=bang1.ngay_sinh`|
|5|gioi_tinh|NUMERIC(1,0)|Giới tính: mã hóa bằng 1 chữ số (Nam = 1 Nữ = 2) |`=bang1.gioi_tinh`|`=bang1.gioi_tinh`|
|6|dia_chi|VARCHAR(1024)|Địa chỉ trên thẻ BHYT đối với trẻ em không có thẻ ghi đầy đủ địa chỉ trên giấy tờ thay thế (tối thiểu phải có địa chỉ về xã huyện tỉnh của trẻ).|`=bang1.dia_chi`|`=bang1.dia_chi`|
|7|ma_the|VARCHAR(50)|Mã thẻ BHYT do cơ quan BHXH cấp không thay đổi không thêm bớt các ký tự|`=bang1.ma_the_bhyt`|`=bang1.ma_the_bhyt`|
|8|ma_dkbd|VARCHAR(50)|Mã cơ sở KCB ban đầu ghi đúng 5 ký tự trên thẻ BHYT|`=bang1.ma_dkbd`|`=bang1.ma_dkbd`|
|9|gt_the_tu|VARCHAR(50)|Thời điểm thẻ có giá trị gồm 8 ký tự; 4 ký tự năm + 2 ký tự tháng + 2 ký tự ngày. Ví dụ: ngày 30/04/2015 được hiển thị là 20150430|`=bang1.gt_the_tu`|`=bang1.gt_the_tu`|
|10|gt_the_den|VARCHAR(50)|Thời điểm thẻ hết giá trị gồm 8 ký tự; 4 ký tự năm + 2 ký tự tháng + 2 ký tự ngày. Ví dụ: ngày 31/05/2015 được hiển thị là 20150531|`=bang1.gt_the_den`|`=bang1.gt_the_den`|
|11|ma_benh|VARCHAR(7)|Mã bệnh chính được mã hóa theo ICD X|`=bang1.ma_benh_chinh`|`=bang1.ma_benh_chinh`|
|12|ma_benhkhac|VARCHAR(100)|Mã bệnh khác mã hóa theo ICD X nếu có nhiều mã ICD thì mỗi mã đuợc phân cách bằng ký tự “;”|`=bang1.ma_benh_kt`|`=bang1.ma_benh_kt`|
|13|ma_lydo_vvien|VARCHAR(4)|Mã hóa lý do đến khám bệnh: 1 = đúng tuyến; 2 = cấp cứu; 3 = trái tuyến|Xét giá trị cột:`bang1.ma_doituong_kcb`[^2024-09-13-04]<br/>➡️`('2')`: `ma_lydo_vvien = '2'`<br/>➡️`('7.1','1.8','1.1','1.4','1.7','1.5','1.3','3.6','3.2')`: `ma_lydo_vvien = '1'`<br/>➡️`('3.3','3.5')`: `ket_qua_dtri = '3'`<br/>➡️`('1.2')`: `ma_lydo_vvien = '4'`<br/>➡️`('1.9','1.10')`: Nếu `bang1.ma_dkbd = bang1.ma_cskcb` thì `ma_lydo_vvien = '1'`, ngược lại `ma_lydo_vvien = '3'`|`=bang1.ma_doituong_kcb`|
|14|ma_noi_chuyen|VARCHAR(5)|Mã cơ sở KCB chuyển người bệnh đến (Mã do cơ quan BHXH cấp)|`=bang1.ma_noi_di`|`=bang1.ma_noi_di`|
|15|ngay_vao|VARCHAR(12)|Thời gian đến khám hoặc nhập viện theo ngày giờ; gồm 12 ký tự; 4 ký tự năm + 2 ký tự tháng + 2 ký tự ngày + 2 ký tự giờ (24 giờ) + 2 ký tự phút. Ví dụ: ngày 30/06/2015 08:20 được hiển thị là 201506300820|`=bang1.ngay_vao`|`=bang1.ngay_vao`|
|16|ngay_ra|VARCHAR(12)|Ngày giờ ra viện; gồm 12 ký tự; 4 ký tự năm + 2 ký tự tháng + 2 ký tự ngày + 2 ký tự giờ (24 giờ) + 2 ký tự phút. Ví dụ: ngày 31/07/2015 16:20 được hiển thị là 201507311620|`=bang1.ngay_ra`|`=bang1.ngay_ra`|
|17|so_ngay_dtri|NUMERIC(3,0)|Số ngày điều trị trong đợt KCB ngoại trú hoặc nằm viện nội trú (= ngày ra - ngày vào). Trường hợp điều trị nội trú nhưng có một số ngày không nằm viện thì tính theo ngày nằm viện thực tế|`=bang1.so_ngay_dtri`|`=bang1.so_ngay_dtri`|
|18|ket_qua_dtri|NUMERIC(1,0)|Kết quả điều trị: Mã hóa (1: Khỏi; 2: Đỡ; 3: Không thay đổi; 4: Nặng hơn; 5: Tử vong)|Xét giá trị cột:`bang1.ket_qua_dtri`[^2024-09-13-03]<br/>➡️`(1)`: `ket_qua_dtri = 1`<br/>➡️`(2)`: `ket_qua_dtri = 2`<br/>➡️`(3, 7)`: `ket_qua_dtri = 3`<br/>➡️`(4, 6)`: `ket_qua_dtri = 4`<br/>➡️`(5, 8)`: `ket_qua_dtri = 5`|`=bang1.ket_qua_dtri`|
|19|tinh_trang_rv|NUMERIC(1,0)|Tình trạng ra viện: Mã hóa (1: Ra viện; 2: Chuyển viện; 3: Trốn viện; 4: Xin ra viện)|`=bang1.ma_loai_rv`[^2024-08-30]|`=bang1.ma_loai_rv`|
|20|t_tongchi|NUMERIC(15,2)|Tổng chi phí KCB BHYT trong lần/đợt điều trị|`=bang1.t_tongchi_bh`|`=bang1.t_tongchi_bh`|
|21|t_xn|NUMERIC(15,2)|Tiền xét nghiệm|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom = 1`|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom = 1`|
|22|t_cdha[^2024-10-04-01]|NUMERIC(15,2)|Tiền chẩn đoán hình ảnh và thăm dò chức năng|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom IN (2,3)`|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom IN (2,3)`|
|23|t_thuoc|NUMERIC(15,2)|Tiền thuốc và dịch truyền|`=SUM(bang2.thanh_tien_bh)` đối với `bang2.ma_nhom = 4`|`=SUM(bang2.thanh_tien_bh)` đối với `bang2.ma_nhom = 4`|
|24|t_mau|NUMERIC(15,2)|Tiền máu và chế phẩm của máu|`=SUM(bang2.thanh_tien_bh)` đối với `bang2.ma_nhom = 7`|`=SUM(bang2.thanh_tien_bh)` đối với `bang2.ma_nhom = 7`|
|25|t_pttt|NUMERIC(15,2)|Tiền phẫu thuật và thủ thuật|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom IN (8,18)`|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom IN (8,18)`|
|26|t_vtyt|NUMERIC(15,2)|Tiền vật tư y tế|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom = 10`|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom = 10`|
...
|30|t_kham|NUMERIC(15,2)|Tiền công khám bệnh ngoại trú hoặc tiền giường nội trú|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom = 13`|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom = 13`|
|31|t_vchuyen|NUMERIC(15,2)|Tiền vận chuyển|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom = 12`|`=SUM(bang3.thanh_tien_bh)` đối với `bang3.ma_nhom = 12`|
|32|t_bntt|NUMERIC(15,2)|Số tiền người bệnh thanh toán (Ghi số tiền người bệnh chi trả bao gồm cùng chi trả tự trả khi khám bệnh chữa bệnh không đúng tuyến số tiền tự trả đối với các dịch vụ y tế áp dụng tỷ lệ thanh toán)|`=bang1.t_bntt + bang1.t_bncct`|`=bang1.t_bntt + bang1.t_bncct`|
|33|t_bhtt|NUMERIC(15,2)|Tiền đề nghị cơ quan BHXH thanh toán (gồm cả chi phí ngoài định suất).|`=bang1.t_bhtt`|`=bang1.t_bhtt`|
|34|t_ngoaids|NUMERIC(15,2)|Tiền thanh toán ngoài định suất (không bao gồm tiền BN cùng chi trả)|`=bang1.t_bhtt_gdv`|`=bang1.t_bhtt_gdv`|
|35|ma_khoa|VARCHAR(50)|Ghi mã khoa theo quy định tại bảng 7 Quyết định số 2348/BYT-BH ngày 10/4/2015 của Bộ Y tế.|`=bang1.ma_khoa`|`=bang1.ma_khoa`|
|36|nam_qt|NUMERIC(4,0)|Năm đề nghị BHXH thanh toán|`=bang1.nam_qt`|`=bang1.nam_qt`|
|37|thang_qt|NUMERIC(2,0)|Tháng đề nghị BHXH thanh toán|`=bang1.thang_qt`|`=bang1.thang_qt`|
|38|ma_khuvuc|VARCHAR(2)|Ghi mã nơi sinh sống trên thẻ BHYT “K1/K2/K3” (nếu có)|`=bang1.ma_khuvuc`|`=bang1.ma_khuvuc`|
|39|ma_loaikcb|VARCHAR(2)|Mã hóa hình thức KCB: (1: khám bệnh; 2: điều trị ngoại trú; 3: điều trị nội trú)|Xét giá trị cột:`bang1.ma_loai_kcb`[^2024-09-13-02]<br/>➡️`('01','05','07','08')`: `ma_loaikcb = '1'`<br/>➡️`('02')`: `ma_loaikcb = '2'`<br/>➡️`('03','04','09')`[^2024-10-16-01]: `ma_loaikcb = '3'`|`=bang1.ma_loai_kcb`|
|40|ma_cskcb|VARCHAR(5)|Mã cơ sở KCB nơi điều trị: Ghi đúng 5 ký tự mã cơ sở KCB do BHXH VN cung cấp|`=bang1.ma_cskcb`|`=bang1.ma_cskcb`|
|41|t_nguonkhac[^2024-08-11]|NUMERIC(15,2)|Ghi tổng số tiền các nguồn khác chi trả ngoài phạm vi chi trả của quỹ BHYT|`=bang1.t_nguonkhac`|`=bang1.t_nguonkhac`|
